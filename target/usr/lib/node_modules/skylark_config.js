(function (fs, net, x2js, child_process, onddclient, telemetry, crypto) {
            ...
            fs.readFile(conf_file_writeable, function(error, data) {
              ...
              if (!error) {
                ...
                data_j_all.overRides.forEach( function(k) {
                  if (resolve(data_j_writeable, k) != undefined )
  ...
  module.exports.setNetConf = function(args, callback) {
    var OthernetPassword = args.OthernetPassword;
    delete args.OthernetPassword;

    // only store pbkdf2 of the wifi sta password - its safer in terms of shell expansions.
    if(args.sta && args.sta.ssid) {
        if (args.sta.password) {
            args.sta.psk = crypto.pbkdf2Sync(args.sta.password,args.sta.ssid,4096,32,'sha1').toString("hex");
        } else {
            args.sta.psk = "";
        }
    }
    ...
    if (OthernetPassword) {
        run_cmd("/usr/bin/changepass.sh",[ "othernet", OthernetPassword] , function() {
